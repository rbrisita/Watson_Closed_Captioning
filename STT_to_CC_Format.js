...
function toClosedCaption(stt, style) {
	if (style === undefined) {
		style = {verbose: true};
		if (style.verbose) {console.log("Using default style");}
	}
	if (style.verbose === undefined) {
		style.verbose = true;
	}
	if (style.verbose) {console.log("Using verbose logging ",Date.now());}
	//if (style.verbose) {console.log(style);}

	if (style.timePerLine === undefined || ((typeof (style.timePerLine) !== "number"))) {
		style.timePerLine = 2.0;
		if (style.verbose) {console.log("Using default line length");}
	}
	if (style.suppressHesitations === undefined || ((typeof (style.suppressHesitations) !== "boolean"))) {
	...
	if (style.outputStyle === undefined) {
		...
		if (style.verbose) {console.log("Using default outputStyle");}
	}
	if ((style.outputStyle !== "SRT") && (style.outputStyle !== "VTT")) {
			style.outputStyle = "SRT";
			console.log("Invalid outputStyle, using default SRT");
	}
	if (style.suppressSpeakerLabels === undefined || ((typeof (style.suppressSpeakerLabels) !== "boolean"))) {
		style.suppressSpeakerLabels = false;
		if (style.verbose) {console.log("Using default suppressSpeakerLabels");}
	}
	...
	if (style.verbose) {console.log("Verbose style = ",style);}
	
	//
	// build a map of speaker labels
	//
	if (stt.speaker_labels) {
		if (style.verbose) {console.log("Speaker Labels were Found");}
		stt.speaker_labels.map(function(speakerLabel) {
			speakers["_"+speakerLabel.from] = speakerLabel.speaker;
		});
	} else {
		if (style.verbose) {console.log("No Speaker Labels Found");}
	}
	...
	if (style.outputStyle === "VTT") {
		cc_out = formatVtt(frames, ((stt.speaker_labels) && (!style.suppressSpeakerLabels)) , cc_out);
	}
	if (style.outputStyle === "SRT") {
		cc_out = formatSrt(frames, ((stt.speaker_labels) && (!style.suppressSpeakerLabels)) , cc_out);
	}
